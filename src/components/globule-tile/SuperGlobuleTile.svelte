...
<GlobuleTileContainer
	name={superGlobuleGeometry.name || ''}
	id={superGlobuleGeometry.superGlobuleConfigId || ''}
	{size}
>
	<button
		on:click={() => {
			onLoad(Number.parseInt(`${superGlobuleGeometry.superGlobuleConfigId}`), false),
				goto('/designer2');
		}}>Load into New Designer</button
	>
	<button
		on:click={() => {
			onLoad(Number.parseInt(`${superGlobuleGeometry.superGlobuleConfigId}`), true),
				goto('/designer2');
		}}>Add to Designer</button
	>

	<button on:click={() => onDelete(Number.parseInt(`${superGlobuleGeometry.superGlobuleConfigId}`))}
		>Delete</button
	>
