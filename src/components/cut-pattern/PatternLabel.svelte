<script lang="ts">
	...
	const centeredOn = (
		...
		const { minX, maxX, minY, maxY } = getPathSize(text);
	...
	const getLabelPath = (value: number, r: number) => {
		...
		// const { path } = centeredOn({ x: (width + padding * 2) / 2, y: 50 }, labelTextPathSegments);

		return svgPathStringFromSegments([
			...stemSegments,
			...labelOutlinePathSegments,
			...translatePS(labelTextPathSegments, 20, 15)
		]);
	};
