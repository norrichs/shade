...
export const distributePanels = (
	...
	if (config.type === 'contiguous') {
		return {
			...pattern,
			tubes: getContiguousDistribution2(pattern.tubes, config)
...
const getContiguousDistribution2 = (
	tubes: TubePanelPattern[],
	config?: DistributionConfig & { type: 'contiguous' }
): TubePanelPattern[] => {
	...
	return tubes.map((tube) => {
		return {
			...
			bands: tube.bands.map((band) => {
				...
				let leadingEdge = {
					anchor: new Vector3(300, 0, 0),
					vector: new Vector3(1, 0, 0),
					p0: bandTrianglePoints[0].base.p0,
					p1: bandTrianglePoints[0].base.p1
				};
				const distributedBand = {
					...
					panels: band.panels.map((panel, p) => {
						...
						leadingEdge = {
							anchor: getTriangleParallelOffset(
								triangle,
								[bandTrianglePoints[p % 2].second.p0, bandTrianglePoints[p % 2].second.p1],
								panelOffset
							),
							vector: getEdgeVector(triangle, [
								bandTrianglePoints[p % 2].second.p0,
								bandTrianglePoints[p % 2].second.p1
							]),
				...
				if (reAlignBands) {
					const bounds = getMinimalBoundingBoxAndRotationAngle(
						getAllTrianglePoints(distributedBand)
					);
...
const reAlignBand = (
	band: BandPanelPattern,
	rotatedCoordinates: { x: number; y: number }[]
): BandPanelPattern => {
	const newBand = {
		...band,
		panels: band.panels.map((panel, panelIndex) => {
			return {
				...
				triangle: new Triangle(
					new Vector3(
						rotatedCoordinates[panelIndex * 3].x,
						rotatedCoordinates[panelIndex * 3].y,
						0
					),
					new Vector3(
						rotatedCoordinates[panelIndex * 3 + 1].x,
						rotatedCoordinates[panelIndex * 3 + 1].y,
						0
					),
					new Vector3(
						rotatedCoordinates[panelIndex * 3 + 2].x,
						rotatedCoordinates[panelIndex * 3 + 2].y,
						0
					)
				)
			};
	...
	const isAscending =
		newBand.panels[0].triangle.a.y < newBand.panels[newBand.panels.length - 1].triangle.a.y;
	if (isAscending) {
...
const getTriangleParallelOffset = (
	triangle: Triangle,
	points: TrianglePointPair,
	offset: number
) => {
...
export const redrawTriangle2 = (
	triangle: Triangle,
	shiftVector: Vector3,
	angle: number,
	anchorPoint: TrianglePoint
): Triangle => {
	...
	newTriangle[anchorPoint] = newTriangle[anchorPoint]
		.addScaledVector(shiftVector, 1)
...
export const getBounds = (
	pattern: TubePanelPattern[]
): {
	bands: { angle: number; width: number; height: number; averageX: number; averageY: number }[];
}[] => {
	const bounds = pattern.map((tube) => ({
		bands: tube.bands.map((band) =>
			getMinimalBoundingBoxAndRotationAngle(getAllTrianglePoints(band))
		)
	}));
...
export const getMinimalBoundingBoxAndRotationAngle = (
	points: Vector3[]
): {
	angle: number;
	width: number;
	...
	for (let angle = 0; angle < Math.PI; angle += angleStep) {
		const { width, height, averageX, averageY, rotatedCoordinates } = getBoundingBoxAtAngle(
			points,
			angle
		);
	...
	return {
		...
		width: minWidth,
		height: bestHeight,
		averageX: bestAverageX,
		averageY: bestAverageY,
		rotatedCoordinates: bestRotatedCoordinates
	};
};

// Helper function to calculate bounding box dimensions at a specific rotation angle
const getBoundingBoxAtAngle = (
	points: Vector3[],
	angle: number
): {
	width: number;
