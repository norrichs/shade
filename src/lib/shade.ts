...
const getLevels = (shadeConfig: LevelConfig[], sides: number): Level[] => {
  ...
  let unitLevel: Level = new Array(sides)
  unitLevel.fill( unitVertex )
  unitLevel = unitLevel.map((vertex, i) => ({
  ...
  levels = levels.map(level => ( level.map(vertex => ({
  ...
  levels = levels.map((level, levelIndex) => {
    ...
    const newLevel: Level = level.map((vertex) => {
      return {
        inner: vertex.inner
          .setLength(lc.r - lc.w)
...
const getRings = (levels: Level[], sides: number, ringOffsetRatio: number): Ring[] => {
  ...
  rings.fill(new Array(sides).fill({inner: new Vector3(), outer: new Vector3()}))
  rings = rings.map((ring, r) => ring.map((vertex, v) => {
...
export const getRibbons = (struts: StrutGroup[][], divisionsByLength: number): RibbonGroup[] => {
  // For divisions by length ===  0, don't convert to curves and interpolate.  
  console.debug("-- getRibbon")
  console.debug("   struts", struts)
  const ribbons: RibbonGroup[] = struts[0].map((strutGroup, sg) => {
    const ribbonGroup: RibbonGroup = {
      angle: strutGroup.angle,
      left: [],
      right: []
    }
    console.debug("  -- group", sg)
    struts.forEach((level, l) => {
      ribbonGroup.right.push(level[sg].right)
      ribbonGroup.left.push(level[(level.length - (l % level.length) + sg) % level.length].left)
    })
    return ribbonGroup

  })
  // 1. sort struts into connected groups
  // 2. push strut points into data structure
  return ribbons
}


export const flattenStruts = (struts: Strut | Strut[], output: "outline" | "triangles", origin: Vector3 = new Vector3(0,0,0)): Strut[] => {
...
export const getShade = (
  ...
  const ribbons = getRibbons(struts, 0)
